...
def game_state(p1, p2):
    ...
    while True:
        ...
        print(f'''{current_player}, it\'s your go now,
              place your {new_game.get_player()}!''')
        turn = input('''Input a column number from 1 (left)
                     to 7 (right) to place your token\n''')
        while turn not in [str(num) for num in range(1, 7)]:
            print("Invalid choice")
            turn = input('''Please input a column number from 1 (left)'
            'to 7 (right) to place your token\n''')
        new_game.play(int(turn)-1)
    print(f"{current_player} won the game!")


# game_state() test text 3
